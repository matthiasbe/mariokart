...
BottleRenderable::BottleRenderable( ShaderProgramPtr shaderProgram, const std::string& filename) :
    MeshRenderable(shaderProgram,filename),
    m_pBuffer(0), m_cBuffer(0), m_nBuffer(0), m_iBuffer(0),update_time(0),lastCollision(0) {
    drink = true;
...
void BottleRenderable::do_animate(float time) {
...
BottleRenderable::~BottleRenderable()
{
...
void BottleRenderable::do_keyPressedEvent(sf::Event& e){
    for(size_t i=0; i<getChildren().size(); ++i)
        getChildren()[i]->keyPressedEvent(e);
}
void BottleRenderable::do_keyReleasedEvent(sf::Event& e)
{
    for(size_t i=0; i<getChildren().size(); ++i)
